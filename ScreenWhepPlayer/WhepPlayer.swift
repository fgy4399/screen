...
final class WhepPlayer: NSObject {
    ...
    private let sdpQueue = DispatchQueue(label: "com.local.ScreenWhepPlayer.WhepPlayer.SDP")
    private let watchdogQueue = DispatchQueue(label: "com.local.ScreenWhepPlayer.WhepPlayer.Watchdog")
    ...
    private var remoteDescriptionStage = "idle"
    ...
    func start(endpoint: WhepEndpoint) {
        workerQueue.async {
            ...
            self.remoteDescriptionStage = "idle"
    ...
    private func addReceiveOnlyTransceivers(to peerConnection: RTCPeerConnection) {
        ...
        recordDebugEvent("Configured video-only receive transceiver")
    }
        ...
        sdpQueue.async { [weak self] in
            ...
            self.workerQueue.async {
                ...
                guard let peerConnection = self.peerConnection else {
                    self.fail(
                        WhepRuntimeError.message("RTCPeerConnection disappeared before applying WHEP answer."),
                        debugExtra: "Missing RTCPeerConnection before setRemoteDescription"
                    )
                    return
                }

                self.recordDebugEvent("Calling setRemoteDescription with \(sanitized.mode.rawValue) SDP")
                self.remoteDescriptionStage = "setting"
                peerConnection.setRemoteDescription(answer) { [weak self] error in
                    ...
                    self.workerQueue.async {
                        ...
                        self.remoteDescriptionStage = "applied"
    ...
    private func describeCurrentPeerConnectionState() -> String {
        ...
        return "signaling=\(describe(peerConnection.signalingState)), ice=\(describe(peerConnection.iceConnectionState)), peer=\(describe(peerConnection.connectionState)), gathering=\(describe(peerConnection.iceGatheringState)), remoteDescriptionStage=\(remoteDescriptionStage), remoteVideo=\(remoteVideoTrack != nil)"
    }
    ...
    private func scheduleRemoteDescriptionTimeout(_ attemptID: Int) {
        watchdogQueue.asyncAfter(deadline: .now() + 5) { [weak self] in
            guard let self,
                  self.remoteDescriptionAttemptID == attemptID else {
                return
            }

            var shouldFail = false
            self.workerQueue.sync {
                shouldFail = self.remoteDescriptionAttemptID == attemptID
                    && self.peerConnection?.remoteDescription == nil
            }
            ...
            self.workerQueue.async {
                guard self.remoteDescriptionAttemptID == attemptID,
    ...
    private func sanitizeAnswerSDP(_ sdp: String, mode: AnswerSanitizationMode) -> SanitizedAnswer {
        ...
        var shouldKeepCurrentMedia = true
        ...
        for rawLine in sdp.split(separator: "\n", omittingEmptySubsequences: false).map(String.init) {
            ...
            if line.hasPrefix("m=") {
                currentMLineIndex += 1
                shouldKeepCurrentMedia = line.hasPrefix("m=video ")
                guard shouldKeepCurrentMedia else {
                    currentMid = nil
                    continue
                }
            ...
            if currentMLineIndex >= 0, !shouldKeepCurrentMedia {
                continue
            }
